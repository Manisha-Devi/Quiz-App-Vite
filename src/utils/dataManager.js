...
const forceCloseConnections = async () => {
  try {
    if (db && !db.closed) {
      db.close();
      db = null;
...
// Initialize database only when explicitly needed (not after deletion)
const initializeDatabase = async () => {
  // Don't auto-recreate if database was intentionally deleted
  if (db && db.closed) {
    console.log('Database connection was closed, not auto-recreating');
    return null;
  }
  ...
  if (!db) {
    try {
      ...
      console.log('Database not available (possibly deleted):', error.message);
      return null;
    }
  }
  return db;
...
const getExamData = async (examId) => {
  try {
    ...
    if (!database) {
      console.log('Database not available, cannot fetch exam data');
      return null;
    }
    return await getData('examData', examId);
  } catch (error) {
...
const saveExamData = async (examId, data) => {
  try {
    const database = await initializeDatabase();
    if (!database) {
      console.log('Database not available, cannot save exam data');
      return false;
    }
    return await storeData('examData', { id: examId, ...data });
  } catch (error) {
...
const getUserSettings = async (userId = 'default') => {
  try {
    const database = await initializeDatabase();
    if (!database) {
    ...
    return await getData('userSettings', userId);
  } catch (error) {
...
const saveUserSettings = async (userId = 'default', settings) => {
  try {
    const database = await initializeDatabase();
    if (!database) {
      console.log('Database not available, cannot save settings');
      return false;
    }
    return await storeData('userSettings', { id: userId, ...settings });
  } catch (error) {
...
const getExamResults = async (examId) => {
  try {
    const database = await initializeDatabase();
    if (!database) {
      console.log('Database not available, cannot fetch results');
      return null;
    }
    return await getData('examResults', examId);
  } catch (error) {
...
const saveExamResults = async (examId, results) => {
  try {
    const database = await initializeDatabase();
    if (!database) {
      console.log('Database not available, cannot save results');
      return false;
    }
    return await storeData('examResults', { id: examId, ...results });
  } catch (error) {
